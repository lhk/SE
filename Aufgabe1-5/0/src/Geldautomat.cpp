...
string Geldautomat::zinsenAusdrucken() {
	...
	while (iterator != konten.end()) {
		...
		gesammtZinsen = gesammtZinsen + r;

		iterator++;
	}

	result << "---------------------" << endl;
	result << "Gesamtzinsen: " << gesammtZinsen << endl;
	result << "---------------------" << endl;
...
double Geldautomat::getGesammtZinsen() {
	return this->gesammtZinsen;
}
